...
app.post('/register', function(request, response, next){
  // Dans la requête on devrait retrouver un JSON de la forme :
  /*
  * {
  *    firstname: un prenom,
  *    lastname: un nom,
  *    birthdate: une date de naissance,
  *    email: uneadresseemail@email.com
  *    password: un mot de passe
  * }
  */
  // Attention à vérifier que les propriété de body existent avant de les utiliser...
  ...
  request.pg.client.query('INSERT INTO users(firstname, lastname, birthdate, email, password) VALUES($1, $2, $3, $4, $5) RETURNING *', [
    firstname,
    lastname,
    birthdate,
    email,
    ...
    if (error) {
      ...
      // Si un utilisateur a bien été créé.
      // console.log(results.rows[0]);
      // On créé le token jwt avec les données utilisateur.
      const token = jwt.sign(results.rows[0], privateKey);
      // On envoi le token au client.
      return response
        .json({
          token: token
        })
